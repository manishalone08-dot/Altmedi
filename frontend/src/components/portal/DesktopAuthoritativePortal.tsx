...
import { POSConnectorPanel } from './POSConnectorPanel';
import { ABDMIntegrationPanel } from './ABDMIntegrationPanel';
...
export const DesktopAuthoritativePortal: React.FC<DesktopPortalProps> = ({
  ...
  const [adminSubTab, setAdminSubTab] = useState<'governance' | 'tenants' | 'pos' | 'abdm'>('governance');
      ...
      {(activeRole === 'platform_admin' || activeRole === 'tenant_admin') && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2 border-b border-slate-200 pb-3">
            ...
            >
              Pharmacy POS Connectors
            </button>
            <button
              type="button"
              onClick={() => setAdminSubTab('abdm')}
              className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                adminSubTab === 'abdm'
                  ? 'bg-teal-700 text-white shadow-xs'
                  : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50'
            ...
            >
              ABDM National Compliance
            </button>
          </div>

          {/* Sub-Tab 1: Governance & Audit */}
          {adminSubTab === 'governance' && (
            <div className="space-y-6">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
                <div className="p-4 border-b border-slate-100 flex items-center justify-between">
                          ...
                          <td className="px-4 py-3 text-right space-x-1.5">
                            {map.status !== 'approved' && (
                              <button
                                ...
                                className="px-2 py-1 rounded bg-emerald-600 text-white font-semibold text-[11px] cursor-pointer"
                              >
                                Approve
                              </button>
                            )}
                            {map.status !== 'quarantined' && (
                              <button
                                ...
                                className="px-2 py-1 rounded bg-rose-600 text-white font-semibold text-[11px] cursor-pointer"
                              >
                                Quarantine
                              </button>
                            )}
          ...
          {/* Sub-Tab 2: Multi-Region Tenants */}
          {adminSubTab === 'tenants' && <TenantAdminDashboard />}

          {/* Sub-Tab 3: Pharmacy POS Connectors */}
          {adminSubTab === 'pos' && <POSConnectorPanel />}

          {/* Sub-Tab 4: ABDM National Compliance */}
          {adminSubTab === 'abdm' && <ABDMIntegrationPanel />}
        </div>
      )}
